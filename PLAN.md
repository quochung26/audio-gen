...
- **Trang truyện** `/story/[slug]`: ảnh bìa, mô tả, danh sách tập, nút Phát / Phát tiếp.
- **Trang nghe** `/listen/[id]`: player lớn + văn bản chạy theo audio (karaoke-style).
...
| **7 — Truyện dài & series** (1 tuần) | Quản lý bộ/tập, tóm tắt tự động, chạy hàng loạt, RSS podcast. **Xong** (README đánh số là Phase 6): chạy hàng loạt điều phối bằng sự kiện ở worker, RSS podcast `/story/<slug>/rss.xml`. | Đặt lệnh 10 tập → ~40 phút có bản thảo giữ đúng nhân vật. |
